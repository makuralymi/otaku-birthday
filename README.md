...
- 🌊 **无级平滑滚动 + 边缘回弹**：主页面接管滚轮，把每一格输入平滑成连续滑动（无级；临界阻尼跟随，中部不回弹）；只有**到顶/到底**才出现橡皮筋拉扯与松手回弹。抽屉等内部滚动区一律交还原生滚动，键盘/滚动条/锚点不受影响。参数在 `src/lib/scrollSmooth.js` 顶部（`OMEGA` / `MAX_RUBBER` / `RUBBER_ZETA`）
...
```bash
npm test            # 常规档：108 项
CLEAN=1 npm test    # 干净档：106 项（断言「全站 0 个外链」「没有分享按钮」「仍标注数据源」等）
```
...
108 项断言覆盖：下拉与人数、URL 同步、卡片渲染与纯色变量、日历 366 天纯色分级、类型筛选、
...
- **无级平滑**：主页面接管 `wheel`（`preventDefault` 后由自己逐帧驱动），把每一格输入平滑成约 200ms 的连续滑动，
  而不是让页面瞬移；跟随用**临界阻尼**系统 `a = ω²(target−cur) − 2ω·vCur`（ζ=1）——
  只追平目标、绝不过冲；
...
| 场景 | 结果 |
| --- | --- |
| 单格滚轮（deltaY=120） | `0 → 46 → 73 → 102 → 109 → 116 → 119 → 120`，12 帧中间态（无级） |
