...
- 🌊 **滚动平滑（弹簧模型）**：原生滚动不动分毫，只读 `scrollY` 差分出速度，再由二阶弹簧-阻尼系统给内容一个「跟得上但略滞后」的偏移 —— 起步柔和、停下时还会回弹一点（惯性）。手感参数集中在 `src/lib/scrollSpring.js` 顶部（`OMEGA` / `ZETA` / `GAIN` / `MAX_OFFSET`）
- 🃏 **卡片浮出**：`class="grid"` 里的卡片滚到视口时才浮出（带错峰延迟）
...
历史上试过两条错路，都已删除：① 劫持滚轮 + 自己插值滚动（改写了触控板惯性、键盘、滚动条，抽屉内滚动也被拦）；
② 位置滞后 lerp（一阶低通：滞后量随累计滚动距离增长，滚得越远越脱节）。

现在的做法**完全不碰原生滚动**：

- 不监听 `wheel`、不 `preventDefault`：滚轮 / 触控板惯性 / 键盘 / 滚动条拖动 / 锚点跳转全是浏览器原生行为，
  抽屉（`.drawer-panel`）这类自滚动区域完全不受影响；
- 每帧只读 `window.scrollY`，用帧间位移差分出瞬时滚动速度，再取**最近 5 帧均值**去抖
  （滚轮是一格一格跳的，原始速度很毛）；
- 速度 → 目标偏移 `target = clamp(v * GAIN, ±MAX_OFFSET)`：滚得越快，内容越「落后」一点点；
- 偏移量 `x` 由二阶系统平滑逼近 target：`a = ω²(target − x) − 2ζω·vx`，
  `ω=11`、`ζ=0.55`（欠阻尼）→ 起步柔和、停手后带一点**回弹**，这就是「一点惯性」；
- 偏移有硬上限 `MAX_OFFSET=34px`；回到静止阈值（`|x| < 0.35px` 且 `|vx| < 0.8`）立即归零、
  清掉 transform、停掉 rAF（静止时不留合成层、不占主线程）；
- `prefers-reduced-motion: reduce` 或触摸为主的设备整段跳过；
- 偏移是纯视觉的 `translate3d`，只作用 `.page`，而 `.page` **不含**任何 `position: fixed` 元素
  （顶栏 / 开屏 / 抽屉 / Toast 都在容器外），所以 fixed 定位不会被 transform 变成包含块；
  抽屉面板另加 `data-native-scroll="1"` 与 `overscroll-behavior: contain`。

无头 Firefox 探针实测（持续滚动 30×40px/20ms ≈ 2000px/s，再停手观察）：

| 场景 | 结果 |
| --- | --- |
| 原生滚动是否被劫持 | `scrollY` 精确到 1200，停止后不漂移 |
| 滚动中的偏移曲线 | `0.1 → 3.0 → 14.2 → 23.9 → 29.3 → 30.8px`（平滑升起） |
| 峰值偏移 | 30.8px（有阻尼感，且 ≤ 34px 上限） |
| 停止后的偏移曲线 | `25.7 → 23.1 → 14.8 → 7.9 → 1.0 → **−2.3 → −3.0** → −1.3 → −0.6 → 0` |
| 回弹幅度 | 约 **3px**（欠阻尼带来的惯性手感） |
| 归零 | 可见运动 ~550ms 结束，~960ms 清空 transform（此后零开销） |
| 大跳转（0 → 4000px 一帧内） | 峰值仅 11.0px（被限幅），随后同样归零 |
